...
**NEW!** 日本語の変数名・関数名（パラグラフ名）をサポートするためのデモンストレーション用ファイルです。

### 日本語文字サポート機能

この拡張機能は、Shift-JISエンコーディングの日本語文字を含む以下の機能をサポートします：

1. **日本語変数名のサポート**:
   - `顧客情報`, `顧客番号`, `顧客名` などの日本語変数名を定義できます
   - データ項目のPIC句やレベル番号も正しく認識されます
   - アウトラインビューで日本語変数が階層表示されます

2. **日本語パラグラフ名のサポート**:
   - `初期化処理`, `データ入力処理`, `計算処理` などの日本語パラグラフ名を使用できます
   - PERFORM文で日本語パラグラフ名を参照できます
   - Ctrl+Clickで日本語パラグラフへジャンプできます

3. **定義ジャンプ**:
   - 日本語の変数名やパラグラフ名でもCtrl+Clickで定義元にジャンプできます
   - PERFORM文から日本語パラグラフへの参照ジャンプが可能です

4. **ホバー表示**:
   - 日本語変数名にマウスカーソルを置くと、レベル番号、PIC句、定義位置が表示されます

5. **IntelliSense（自動補完）**:
   - 日本語の変数名やパラグラフ名も補完候補に表示されます
   - 日本語を入力すると、該当する候補がフィルタリングされます

### 使い方

1. VS Code でこのファイルを開きます
2. 次の操作を試してください：
   - `顧客番号` などの変数名にカーソルを置いてホバー表示を確認
   - `PERFORM 初期化処理` の `初期化処理` をCtrl+Clickしてジャンプ
   - アウトラインビュー (Ctrl+Shift+O) で日本語の階層構造を確認
   - 日本語変数やパラグラフ名の自動補完を試す

### 技術的な詳細

- すべての正規表現パターンが `[\w\u0080-\uFFFF\-]+` に更新され、Unicode文字（日本語を含む）をサポートします
- Shift-JISエンコーディングのファイルを正しく処理できます
- 英数字と日本語が混在した識別子もサポートされます
